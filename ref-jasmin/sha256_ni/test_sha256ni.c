...
extern void sha256_init_jazz(uint32_t *state, uint32_t *datalen, uint8_t *bitlen);
extern void __initH_ref(uint32_t state[8]);
...
void sha256_init(SHA256_CTX *ctx) {
    ...
    // memset(ctx->state, 0, 8 * sizeof(uint32_t));
...
void test_sha256_init(void) {
    ...
    sha256_init(&ctx);                           // reference
    sha256_init_jazz(state, &datalen, &bitlen);  // jasmin

    assert(ctx.datalen == datalen);
    assert(ctx.bitlen == bitlen);
    assert(memcmp(ctx.state, state, 8 * sizeof(uint32_t)) == 0);
}
...
int main(void) {
    run_test(test_sha256_init, "sha256_init", TESTS);
