...
conseq (: 
    ...
    (to_uint i{1} < to_uint upper_bound{1}) /\
    i{2} < 2 ^ t{2} /\ 
    to_uint t32{1} = s{2} + i{2} /\
    val node{2} = to_list buf{1} /\
      ...
      move => H26 H27 result H29 H30 H31 *; do split.
           ...
           case (j = to_uint offset{1}) => [-> |?].
              ...
              - rewrite ifF /=; first by rewrite to_uintD /#.     
                ...
                rewrite -H19 (nth_map witness); first by rewrite size_sub /#.
                by rewrite nth_sub /#.
            ...
            + rewrite n_val; apply (eq_from_nth witness); first by rewrite size_sub 1:/# size_sub_list /#.
              ...
              rewrite nth_sub 1:/# /= /sub_list nth_mkseq 1:/# /= /nbytes_flatten (nth_flatten witness n).
                  * pose P := (fun (s0 : W8.t list) => size s0 = n).
                    ...
                    rewrite -(all_nthP P L witness) /P /L size_map size_put H10 d_val h_val /= => k?. 
                    rewrite (nth_map witness); first by rewrite size_put /#.
                    by rewrite valP.
              rewrite (nth_map witness); first by rewrite size_put /#.
              rewrite nth_put 1:/#.
              case (to_uint offset{1} = j %/ n) => H.
                  * rewrite H25 get_to_list /= -H30 /#.
                  * have ->: nth witness (val (nth witness stack{2} (j %/ n))) (j %% n) = nth witness (sub_list (nbytes_flatten stack{2}) 0 (n * to_uint offset{1})) j.
                      + rewrite /sub_list nth_mkseq 1:/# /= /nbytes_flatten (nth_flatten witness n).
                           - pose P := (fun (s0 : W8.t list) => size s0 = n).
                             ...
                             rewrite -(all_nthP P L witness) /P /L size_map  H10 d_val h_val /= => k?. 
                             by rewrite (nth_map witness) 1:/# valP.
                        by rewrite (nth_map witness) 1:/#.
                    rewrite -H21 nth_sub 1:/# /= /#.
            + rewrite size_put /#.
            + smt().
          ...
          case (0 <= j < 32) => ?; last first.
             - rewrite nth_sub // nth_cat valP ifF 1:/#.
               ...
               rewrite /sub_list nth_mkseq 1:/# /= /nbytes_flatten (nth_flatten witness n).
                   + pose P := (fun (s0 : W8.t list) => size s0 = n).
                     pose L := (map NBytes.val stack{2}).
                     rewrite -(all_nthP P L witness) /P /L size_map H9 d_val h_val /= => k?. 
                     by rewrite (nth_map witness) 1:/# valP.
...
do split.
   ...
   - rewrite to_uintM of_uintK /= to_uintB 2:/# uleE /#.
   - move => H32 H33 result_stack H34 H35 H36 *.
     do split.
         - rewrite to_uintB 2:/# uleE /#.
         - rewrite to_uintB 2:/# uleE /#.
           move => H37 H38 result_cond H39 *.
           rewrite /treehash_cond in H39.
           do split.
             + smt().
             + rewrite to_uintB 2:/# uleE /#.
             + smt().
             ...
             + apply (eq_from_nth witness); first by rewrite size_map size_sub 1:/# size_sub_list /#.
               ...
               rewrite nth_sub 1:/# get_setE /=.
                    - (* smt nao resolve isto *)
                      rewrite to_uintB; first by rewrite uleE to_uintB 2:/# uleE /#.
                      rewrite to_uintB 2:/# uleE /#.
               rewrite /sub_list nth_mkseq 1:/# /= nth_put 1:/#.
               case (to_uint offset{1} - 2 = j) => H; last by admit.
                    - rewrite ifT.
                         + rewrite to_uintB; first by rewrite uleE to_uintB 2:/# uleE /#.
                           rewrite to_uintB 2:/# uleE /#.
                      have ->: nth witness heights{2} (to_uint offset{1} - 2) = nth witness (sub_list heights{2} 0 (to_uint offset{1})) (to_uint offset{1} - 2).
                      ...
                      have ->: to_uint heights{1}.[to_uint offset{1} - 2] = nth witness (map W32.to_uint (sub heights{1} 0 (to_uint offset{1}))) (to_uint offset{1} - 2).
                         + rewrite (nth_map witness); first by rewrite size_sub /#.
                           rewrite nth_sub /#.
                      rewrite H17 /sub_list nth_mkseq 1:/# /#.
             + move => k??.
               split => *; first by rewrite nth_put /#.
               admit.
             + admit.
             + rewrite size_put /#.
             + admit.
