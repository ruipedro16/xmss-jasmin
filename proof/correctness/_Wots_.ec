...
lemma array3_list_put ['a] (x : 'a Array3.t) (v : 'a) (i : int) : 
    0 <= i => i < 3 =>
    put (to_list x) i v = to_list (x.[i <- v]).
proof.
move => ? ?.
rewrite /put.
case (0 <= i /\ i < size (to_list x)).
move => H ; rewrite ifT ; first by smt().
rewrite /to_list. (* auto => />. *)
pose y :=  (mkseq (fun (i0 : int) => x.[i0]) 3).
admit.
move => H ; rewrite ifF ; first by smt().
smt(@List).
qed.
...
smt(@W64).
smt. (* smt(@W64) fails *)
(* smt(array67_list_put). TODO: Need info about the range of i *)
admit.
qed.
...
auto => /> /#.
while (W64.zero \ult i0) (inlen - (to_uint i0)) => //=.
auto => /> &h.
move => ? ?.  
split.
admit.
