...
module RootFromSig_Hop = {
  proc rootFromSigInnerLoop (nodes0 : nbytes, idx_sig : W32.t, _seed : seed, auth : auth_path, address : adrs) : nbytes * adrs = {
    var k : int;
    var nodes1 : nbytes;
    var index : int;
    var auth_k : nbytes;
  ...
  proc rootFromSig(idx_sig : W32.t, sig_ots : wots_signature, auth : auth_path, M : nbytes, 
    ...
    (nodes0, address) <@ rootFromSigInnerLoop (nodes0, idx_sig, _seed, auth, address);
